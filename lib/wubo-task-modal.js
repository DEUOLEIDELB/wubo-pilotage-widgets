...
(function () {
  ...
  function open(options = {}) {
    ...
    return new Promise(async (resolve) => {
      ...
      // Duree : chips horizontaux scrollables a la place du select. Lecture immediate
      // de la valeur (chiffre grand + unite petite) et hint "→ fin HH:MM" calcule live
      // en fonction de heure_debut + duree pour donner du sens au choix.
      const DUREE_CHOIX = [
        { v: null, label: '—', sub: 'sans' },
        { v: 15,   label: '15', sub: 'min' },
        { v: 30,   label: '30', sub: 'min' },
        { v: 45,   label: '45', sub: 'min' },
        { v: 60,   label: '1', sub: 'h' },
        { v: 90,   label: '1:30', sub: 'h' },
        { v: 120,  label: '2', sub: 'h' },
        { v: 180,  label: '3', sub: 'h' },
        { v: 240,  label: '4', sub: 'h' }
      ];
      ...
      function paintDureeChips() {
        ...
        DUREE_CHOIX.forEach(d => {
          ...
          btn.appendChild($el('span', { class: 'task-duree-chip-num' }, d.label));
          btn.appendChild($el('span', { class: 'task-duree-chip-unit' }, d.sub));
          dureeChipsRow.appendChild(btn);
        });
      }
      paintDureeChips();
      paintDureeFinHint();
