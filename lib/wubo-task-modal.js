...
(function () {
  ...
  //   - Si 0 ou 1 bloc dispo : auto-place
  //   - Si 2+ blocs dispo OU tous bloques : NE LIE PAS (la tache reste flottante chez l'assigne)
  async function autoLinkToAssigneeBlocs(taskId, dateUnix, assignees, preferredTranche, selfOrdre = 1) {
    const undoInfo = { taskId, autoCreatedBlocs: [], linkedBlocs: [], unplaced: [], blocked: [], private: [] };
    ...
    for (const user of assignees) {
      try {
        ...
        if (result && result.blocked) {
          undoInfo.blocked.push({ user });
          continue;
        }
        if (result && result.private) {
          undoInfo.private.push({ user });
          continue;
        }
        if (result && result.ambiguous) {
          undoInfo.unplaced.push({ user, count: result.count, allBlocked: result.allBlocked });
          continue;
        }
        if (!result) continue;
