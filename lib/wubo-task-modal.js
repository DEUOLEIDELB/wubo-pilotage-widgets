        ...
        onclick: async () => {
          ...
          try {
            if (mode === 'edit' && task && task.id) {
              ...
              // Si blocId explicite (ex: "+ tâche" sur un bloc spécifique) on l'utilise pour current user
              // et l'auto-link gère uniquement les autres.
              const currentUser = WuboGrist.getUser() || 'Taki';
              ...
              if (allAssignees.length > 0 && dateUnix) {
                // Si current user a déjà été lié via blocId, on ne le re-link pas
                const toLink = blocId ? otherAssignees : allAssignees;
                if (toLink.length > 0) {
                  undoInfo = await autoLinkToAssigneeBlocs(newId, dateUnix, toLink);
                }
              }
