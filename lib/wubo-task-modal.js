    ...
    return new Promise(async (resolve) => {
      ...
      if (mode === 'edit' && task && task.id && window.WuboGCal) {
        ...
        if (showPush || showDrop) {
          ...
          // Bouton 'Délier' supprimé : la tâche déliée serait juste ré-importée au
          // prochain sync (l'event existe encore dans gCal). Pas de cas d'usage clean.
          // Pour stopper la sync sur une tâche : la supprimer dans gCal directement
          // — Pilotage suivra automatiquement (passage en Archive).
      ...
      if (mode === 'edit' && task && task.id) {
        stickySave.appendChild($el('button', {
          class: 'danger small',
          onclick: async () => {
            // Si la tache vient de gCal, on previent : la suppression cote Pilotage
            // ne touche pas gCal. L'event reviendra au prochain sync. Pour eviter ca,
            // l'user doit aussi supprimer l'event cote gCal.
            const isGCalSynced = !!task.gcal_event_id;
            const message = isGCalSynced
              ? `"${task.titre || '(sans titre)'}". Cette tâche vient de Google Calendar. Si tu ne supprimes pas aussi l'event dans gCal, elle reviendra au prochain refresh.`
