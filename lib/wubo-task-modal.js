...
(function () {
  ...
  function open(options = {}) {
    ...
    return new Promise(async (resolve) => {
      ...
      // Section Google Calendar : push (proprio connecte avec date_cible) +
      // delier (si tache deja liee). On regroupe les deux pour eviter d'avoir
      // deux sections "Google Calendar" successives quand les deux conditions
      // sont remplies. Affichee seulement en mode edit.
      if (mode === 'edit' && task && task.id && window.WuboGCal) {
        const currentUser = WuboGrist.getUser();
        const isOwner = task.proprietaire === currentUser;
        const gcalConnected = isOwner && WuboGCal.isConnected(currentUser);
        const hasDate = !!task.date_cible;
        const showPush = isOwner && gcalConnected && hasDate;
        const showDrop = !!task.gcal_event_id;

        if (showPush || showDrop) {
          ...
          if (showPush) {
            const pushBtnLabel = task.gcal_event_id
              ? 'Re-sync vers Google Calendar'
              : 'Pousser vers Google Calendar';
            const pushBtn = $el('button', {
              type: 'button',
              class: 'ghost small',
              style: 'align-self:flex-start',
              onclick: async () => {
                const wasUpdate = !!task.gcal_event_id;
                pushBtn.disabled = true;
                const originalLabel = pushBtn.textContent;
                pushBtn.textContent = wasUpdate ? 'Re-sync...' : 'Envoi...';
                try {
                  const res = await window.WuboGCal.pushTask(currentUser, task);
                  WuboGrist.toast(
                    wasUpdate ? 'Event Google Calendar mis à jour' : 'Tâche poussée vers Google Calendar',
                    'success'
                  );
                  if (!wasUpdate && res && res.eventId) {
                    task.gcal_event_id = res.eventId;
                  }
                  ...
                  if (e && e.message === 'TOKEN_EXPIRED') {
                    WuboGrist.toast('Session Google expirée, reconnecte-toi', 'warn');
                  } else {
                    WuboGrist.toast('Erreur push : ' + (e && e.message || 'inconnue'), 'error');
                  }
                }
              }
            }, pushBtnLabel);
            gcalWrap.appendChild(pushBtn);
          }
      ...
      if (mode === 'edit' && task && task.id) {
        stickySave.appendChild($el('button', {
          class: 'danger small',
          onclick: async () => {
            // Si la tache est synced gCal, on previent le user que l'event sera
            // supprime cote gCal aussi (sinon il reviendrait au prochain sync).
            const isGCalSynced = !!(task.gcal_event_id && window.WuboGCal);
            const currentUser = WuboGrist.getUser() || 'Taki';
            const canDeleteGCal = isGCalSynced
              && task.proprietaire === currentUser
              && WuboGCal.isConnected(currentUser);
            const message = canDeleteGCal
              ? `"${task.titre || '(sans titre)'}". L'event sera aussi supprimé dans Google Calendar. Irréversible.`
              : `"${task.titre || '(sans titre)'}", irréversible.`;
            const ok = await WuboUI.confirm({
            ...
            if (!ok) return;
            try {
              // 1. Supprime cote gCal d'abord (si applicable). Si echec, on stoppe :
              // sinon la tache reviendrait au prochain sync de la fenetre.
              if (canDeleteGCal) {
                try {
                  await WuboGCal.deletePushedEvent(
                    currentUser,
                    task.gcal_event_id,
                    task.gcal_calendar_id || 'primary'
                  );
                } catch (e) {
                  if (e.message === 'TOKEN_EXPIRED') {
                    WuboGrist.toast('Session Google expirée, reconnecte-toi puis réessaie', 'error');
                    return;
                  }
                  // Autres erreurs (404 = deja supprime cote gCal, on continue)
                  if (!String(e.message).includes('404')) {
                    WuboGrist.toast('Erreur gCal : ' + e.message + ' (tâche non supprimée)', 'error');
                    return;
                  }
                }
              }
              // 2. Supprime cote Pilotage
              await WuboGrist.deleteRecord('Taches', task.id);
              WuboGrist.toast(canDeleteGCal ? 'Tâche supprimée (Pilotage + gCal)' : 'Tâche supprimée', 'info');
