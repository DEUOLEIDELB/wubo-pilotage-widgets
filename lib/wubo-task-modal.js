...
(function () {
  ...
  // Pour chaque assignee != currentUser, trouve/crée son bloc à la date de la tâche
  // et y lie la tâche. Retourne info pour pouvoir undo.
  async function autoLinkToAssigneeBlocs(taskId, dateUnix, assignees) {
    const currentUser = WuboGrist.getUser() || 'Taki';
    const others = (assignees || []).filter(u => u !== currentUser);
    const undoInfo = { taskId, autoCreatedBlocs: [], linkedBlocs: [] };
    if (!others.length || !dateUnix) return undoInfo;

    for (const user of others) {
      try {
        const { blocId, created } = await ensureBlocForUserAtDate(user, dateUnix);
  ...
  function open(options = {}) {
    ...
    return new Promise(async (resolve) => {
      ...
      if (task && task.date_cible) dateInput.value = unixToInputDate(task.date_cible);
        ...
        onclick: async () => {
          ...
          try {
            if (mode === 'edit' && task && task.id) {
              ...
              // Auto-link aux blocs des assignees autres que current user
              const currentUser = WuboGrist.getUser() || 'Taki';
              const otherAssignees = (collabs || []).slice(1).filter(u => u !== currentUser);
              let undoInfo = null;
              if (otherAssignees.length > 0 && dateUnix) {
                undoInfo = await autoLinkToAssigneeBlocs(newId, dateUnix, [...otherAssignees]);
              }
              ...
              // Si assignation cross-user : afficher cooldown notice 10s pour Modifier/Annuler
              if (undoInfo && otherAssignees.length > 0 && window.WuboUI && WuboUI.cooldownNotice) {
                const names = otherAssignees.join(', ');
                const blocsCreated = undoInfo.autoCreatedBlocs.length;
                const msg = blocsCreated > 0
                  ? `Assigné à ${names}. Bloc créé chez ${otherAssignees.length > 1 ? 'eux' : names}.`
                  : `Assigné à ${names}.`;
                WuboUI.cooldownNotice({
                  message: msg,
                  durationMs: 10000,
                  onCancel: async () => {
                    await undoAssignment(undoInfo);
                    WuboGrist.toast('Assignation annulée', 'info');
                  },
                  onEdit: async () => {
                    // Réouvre la modal sur la tâche fraichement créée
                    try {
