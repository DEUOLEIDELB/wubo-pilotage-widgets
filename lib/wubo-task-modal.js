...
(function () {
  ...
  // Pour chaque assignee, lie la tache au bon bloc :
  // - Pour le current user (s'il est dans assignees) : utilise selfOrdre (du sub-picker, defaut 1)
  // - Pour les autres : appelle planTaskInTranche
  //   - Si 0 ou 1 bloc dans la tranche : auto-place
  //   - Si 2+ blocs : NE LIE PAS (la tache restera flottante chez l'assigné, qui choisira)
  async function autoLinkToAssigneeBlocs(taskId, dateUnix, assignees, preferredTranche, selfOrdre = 1) {
    const undoInfo = { taskId, autoCreatedBlocs: [], linkedBlocs: [], unplaced: [] };
    ...
    for (const user of assignees) {
      try {
        let result;
        if (user === currentUser) {
          // Current user : on connait le bloc precis (sub-picker)
          result = await WuboGrist.planTaskInBloc(taskId, dateUnix, user, preferredTranche, selfOrdre);
        } else {
          result = await WuboGrist.planTaskInTranche(taskId, dateUnix, user, preferredTranche);
        }
        if (result && result.ambiguous) {
          // Plusieurs blocs cibles → tache laissee sans lien chez ce user
          undoInfo.unplaced.push({ user, count: result.count });
          continue;
        }
        if (!result) continue;
        const { blocId, created } = result;
        if (created) undoInfo.autoCreatedBlocs.push(blocId);
        // planTaskInBloc/planTaskInTranche font deja le lien — on note juste pour undo
  ...
  function open(options = {}) {
    ...
    return new Promise(async (resolve) => {
      ...
      [
        ...
        trancheSelect.appendChild($el('option', { value: v }, lbl));
      });

      // Sub-picker : bloc precis dans la tranche, visible seulement quand
      // le current user a 2+ blocs dans la tranche choisie. Sinon ordre=1 implicite.
      const blocPrecisSelect = $el('select', {});
      const blocPrecisField = $el('div', { class: 'field', style: 'display:none' },
        $el('label', {}, 'Dans quel bloc ?'),
        blocPrecisSelect
      );

      content.appendChild($el('div', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:8px' },
        field('Échéance', dateInput),
        field('Bloc', trancheSelect)
      ));
      content.appendChild(blocPrecisField);

      // Met a jour le sub-picker selon la tranche choisie + profil current user.
      // Affiche uniquement si current user a 2+ blocs dans la tranche.
      async function refreshBlocPrecisPicker(preselectedOrdre) {
        const currentUser = WuboGrist.getUser() || 'Taki';
        const tranche = trancheSelect.value;
        if (!tranche) {
          blocPrecisField.style.display = 'none';
          return;
        }
        try {
          const profilBlocs = await WuboGrist.getProfilBlocsByTranche(currentUser, tranche);
          if (profilBlocs.length < 2) {
            blocPrecisField.style.display = 'none';
            return;
          }
          // 2+ blocs : on affiche le sub-picker
          while (blocPrecisSelect.firstChild) blocPrecisSelect.removeChild(blocPrecisSelect.firstChild);
          profilBlocs.forEach(pb => {
            const time = WuboGrist.formatBlocTime(pb.debut_heure, pb.fin_heure);
            const lbl = time ? `${pb.label} · ${time}` : pb.label;
            const opt = $el('option', { value: String(pb.ordre || 1) }, lbl);
            if (preselectedOrdre && (pb.ordre || 1) === preselectedOrdre) opt.selected = true;
            blocPrecisSelect.appendChild(opt);
          });
          blocPrecisField.style.display = '';
        } catch (_) {
          blocPrecisField.style.display = 'none';
        }
      }
      trancheSelect.addEventListener('change', () => refreshBlocPrecisPicker());
      ...
      content.appendChild(field('Heures', heuresInput));

      // Variables pour pre-fill et detection de changements
      let originalTranche = '';
      let originalOrdre = 1;
      const originalAssignees = (task && WuboGrist.getAssignees) ? WuboGrist.getAssignees(task).slice() : [];

      // Pre-fill tranche + ordre intra-tranche selon le contexte
      (async () => {
        let prefillOrdre = null;
        if (mode === 'edit' && task && task.id) {
          try {
            const blocs = await WuboGrist.getAllBlocsForTask(task.id);
            const currentUser = WuboGrist.getUser() || 'Taki';
            const mine = blocs.find(b => b.proprietaire === currentUser);
            const fallback = blocs[0];
            const target = mine || fallback;
            if (target && target.tranche) {
              originalTranche = target.tranche;
              originalOrdre = target.tranche_ordre || 1;
              prefillOrdre = originalOrdre;
              trancheSelect.value = target.tranche;
            }
          } catch (_) {}
        } else if (isNew && blocId) {
          // Create depuis un bloc precis : pre-fill avec sa tranche ET son ordre
          try {
            const blocs = await WuboGrist.fetchRows('Blocs_temps');
            const orig = blocs.find(b => b.id === blocId);
            if (orig && orig.tranche) {
              trancheSelect.value = orig.tranche;
              prefillOrdre = orig.tranche_ordre || 1;
            }
          } catch (_) {}
        }
        await refreshBlocPrecisPicker(prefillOrdre);
      })();
      ...
      stickySave.appendChild($el('button', {
        ...
        onclick: async () => {
          ...
          // Tranche + ordre intra-tranche pour le current user
          const selectedTranche = trancheSelect.value || '';
          // Sub-picker visible = current user a 2+ blocs dans la tranche → ordre choisi.
          // Sinon ordre=1 (comportement par defaut, equivalent a aucune ambiguite).
          const subPickerVisible = blocPrecisField.style.display !== 'none';
          const selfOrdre = subPickerVisible ? (parseInt(blocPrecisSelect.value, 10) || 1) : 1;
          ...
          try {
            if (mode === 'edit' && task && task.id) {
              ...
              // Detection des changements qui imposent un rebinding bloc
              const dateChanged = (originalDate || 0) !== (dateUnix || 0);
              const trancheChanged = (originalTranche || '') !== (selectedTranche || '');
              const ordreChanged = originalOrdre !== selfOrdre;
              const sameSet = (a, b) => a.length === b.length && a.every(x => b.includes(x));
              const assigneesChanged = !sameSet(originalAssignees, [propriotaireValue, ...collabsList].filter((v, i, arr) => v && arr.indexOf(v) === i));

              if (dateChanged || trancheChanged || ordreChanged || assigneesChanged) {
                const oldBlocs = await WuboGrist.getAllBlocsForTask(task.id);
                await WuboGrist.unlinkTaskFromBlocs(task.id, oldBlocs);
                if (selectedTranche && dateUnix && allAssigneesNew.length > 0) {
                  await autoLinkToAssigneeBlocs(task.id, dateUnix, allAssigneesNew, selectedTranche, selfOrdre);
                }
              }
              ...
              // Bloc binding : seulement si tranche choisie + date posee
              let undoInfo = null;
              if (selectedTranche && dateUnix && allAssigneesNew.length > 0) {
                undoInfo = await autoLinkToAssigneeBlocs(newId, dateUnix, allAssigneesNew, selectedTranche, selfOrdre);
              }
              ...
              } else if (!selectedTranche) {
                WuboGrist.toast('Tâche créée. À planifier dans Semaine.', 'info', 3500);
              } else {
                WuboGrist.toast('Tâche créée', 'success');
              }

              // Cooldown notice 10s si assignation cross-user ET tranche choisie
              if (otherAssigneesNew.length > 0 && selectedTranche && window.WuboUI && WuboUI.cooldownNotice) {
                const names = otherAssigneesNew.join(' & ');
                const blocsCreated = undoInfo ? undoInfo.autoCreatedBlocs.length : 0;
                const unplaced = undoInfo ? undoInfo.unplaced : [];
                let msg;
                if (unplaced.length > 0) {
                  // Assigné a un user qui a 2+ blocs Matin → tache reste sans lien chez lui
                  const unplacedNames = unplaced.map(u => u.user).join(' & ');
                  msg = `Assigné à ${names} en ${selectedTranche}. ${unplacedNames} choisira son bloc précis.`;
                } else if (blocsCreated > 0) {
                  msg = `Assigné à ${names}. Bloc ${selectedTranche} créé.`;
                } else {
                  msg = `Assigné à ${names} en ${selectedTranche}.`;
                }
