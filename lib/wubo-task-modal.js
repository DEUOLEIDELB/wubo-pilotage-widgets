...
(function () {
  ...
  // Trouve ou crée un bloc pour un user à une date donnée + tranche demandée.
  // Si preferredTranche est fournie, on cherche EXACTEMENT cette tranche, sinon on en crée
  // une nouvelle (jamais de fallback sur un autre bloc du même jour).
  async function ensureBlocForUserAtDate(user, dateUnix, preferredTranche) {
    ...
    let existing;
    if (preferredTranche) {
      // Match strict : si on veut Soir, on prend Soir (ou rien → on en crée un)
      existing = userBlocs.find(b => b.tranche === preferredTranche);
    } else {
      // Pas de tranche demandée → on prend le premier bloc du jour s'il existe
  ...
  function open(options = {}) {
    ...
    return new Promise(async (resolve) => {
      ...
      // Pré-fill tranche : voir étape 3 plus bas (après création des éléments)
      // — initialisation différée car peut nécessiter un fetch async
      ...
      // Pre-fill tranche selon le contexte
      (async () => {
        if (mode === 'edit' && task && task.id) {
          // Edit : cherche le bloc actuel pour current user (si plusieurs : prend le premier)
          try {
          ...
          // Create depuis un bloc précis : pré-fill avec sa tranche
          try {
        ...
        // Sinon : reste à '' (à planifier plus tard) — comportement voulu pour création depuis Dashboard
      })();
      ...
      stickySave.appendChild($el('button', {
        ...
        onclick: async () => {
          ...
          // Tranche choisie explicitement (pilote le bloc binding)
          ...
          try {
            if (mode === 'edit' && task && task.id) {
              ...
              // Comparaison set d'assignees (ordre indifferent)
              ...
              if (dateChanged || trancheChanged || assigneesChanged) {
                // Detache la tache de tous les blocs ou elle etait
                const oldBlocs = await WuboGrist.getAllBlocsForTask(task.id);
                await WuboGrist.unlinkTaskFromBlocs(task.id, oldBlocs);
                // Re-link si tranche + date + assignees presents
                if (selectedTranche && dateUnix && allAssigneesNew.length > 0) {
                  await autoLinkToAssigneeBlocs(task.id, dateUnix, allAssigneesNew, selectedTranche);
                }
              }
              ...
              // Sinon tache flottante (pas dans Jour, visible uniquement dans Objectifs/Echeances)
              let undoInfo = null;
              if (selectedTranche && dateUnix && allAssigneesNew.length > 0) {
                undoInfo = await autoLinkToAssigneeBlocs(newId, dateUnix, allAssigneesNew, selectedTranche);
              }
              ...
              // (sans tranche, pas de bloc cree pour les autres : rien a annuler de specifique)
              if (otherAssigneesNew.length > 0 && selectedTranche && window.WuboUI && WuboUI.cooldownNotice) {
                ...
                const msg = blocsCreated > 0
                  ? `Assigné à ${names}. Bloc ${selectedTranche} créé.`
                  : `Assigné à ${names} en ${selectedTranche}.`;
