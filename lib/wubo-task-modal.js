        ...
        onclick: async () => {
          ...
          try {
            if (mode === 'edit' && task && task.id) {
              ...
              // DEBUG : montre l'état réel du flow
              WuboGrist.toast(`DBG collabs=[${(collabs||[]).slice(1).join(',')}] others=[${otherAssignees.join(',')}] blocId=${blocId} date=${dateUnix}`, 'info', 6000);
              ...
              if (blocId) {
                try {
                  ...
                  WuboGrist.toast(`DBG origBloc trouvé=${origBloc ? 'oui' : 'NON'} tranche=${preferredTranche}`, 'info', 6000);
                } catch (e) {
                  WuboGrist.toast(`DBG erreur fetch origBloc : ${e.message}`, 'error', 6000);
                }
              }
              ...
              if (allAssignees.length > 0 && dateUnix) {
                ...
                if (toLink.length > 0) {
                  WuboGrist.toast(`DBG autolink → ${toLink.join(',')} en tranche=${preferredTranche || 'Matin (fallback)'}`, 'info', 6000);
                  undoInfo = await autoLinkToAssigneeBlocs(newId, dateUnix, toLink, preferredTranche);
                  WuboGrist.toast(`DBG résultat : ${undoInfo.linkedBlocs.length} bloc(s) lié(s), ${undoInfo.autoCreatedBlocs.length} auto-créé(s)`, 'info', 6000);
                } else {
                  WuboGrist.toast(`DBG toLink vide, skip autolink`, 'warn', 5000);
                }
              } else {
                WuboGrist.toast(`DBG skip autolink : assignees=${allAssignees.length}, dateUnix=${dateUnix}`, 'warn', 5000);
              }
