...
(function () {
  ...
  // ============ Profil utilisateur (multi-user) ============
  // Stocké en localStorage. 3 users possibles : Taki, Numa, Lyes.
  // View mode : 'mine' (moi + Shared, défaut) ou 'team' (tous).
  ...
  function setUser(u) {
    ...
    // Invalide le cache car le filtre va changer
    cacheInvalidateAll();
  }
  function getViewMode() {
    const m = localStorage.getItem('wubo_view_mode');
    return (m === 'team' || m === 'mine') ? m : 'mine';
  }
  function setViewMode(m) {
    localStorage.setItem('wubo_view_mode', m === 'team' ? 'team' : 'mine');
  }
  // Filtre par proprietaire. viewMode 'mine' = user + Shared. 'team' = tout.
  // Robuste aux valeurs manquantes : si proprietaire null → considéré comme Shared (visible par tous).
  function filterByOwner(rows, { field = 'proprietaire' } = {}) {
    const user = getUser();
    const mode = getViewMode();
    if (mode === 'team' || !user) return rows;
    return rows.filter(r => {
      const p = r[field];
      if (!p) return true; // no owner = Shared
      return p === user || p === 'Shared';
    });
  }
  ...
  window.WuboGrist = {
    ...
    USERS, getUser, setUser, getViewMode, setViewMode, filterByOwner
  };
})();
