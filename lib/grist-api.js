// Wubo Pilotage - couche Grist unifiee
...
async function restFetch(path, options = {}) {
  ...
  const res = await fetch(`${GRIST_BASE_URL}/api/docs/${GRIST_DOC_ID}${path}`, {
    ...options,
    headers: {
  ...
  if (res.status === 401 || res.status === 403) {
    resetApiKey();
    throw new Error('Cle API invalide. Recharge la page.');
  }
  if (!res.ok) throw new Error(`Grist ${res.status} : ${await res.text()}`);
  return res.json();
}

// table au format rows [{id, ...fields}]
