...
(function () {
  ...
  // Renvoie les blocs config d'un user, tries par tranche puis ordre intra-tranche.
  ...
  // Match par (tranche, ordre intra-tranche). Priorite : override > profil > fallback.
  ...
  function getBlocDisplay(bloc, profilBlocs) {
    ...
    return { label, debut, fin, isOverride, tranche, ordre };
  }
  ...
  // Toujours rattache a un bloc deterministe — caller doit avoir choisi l'ordre.
  ...
  // - Si user a 0 ou 1 bloc dans la tranche → cree/rattache (ordre=1 ou ordre du seul bloc).
  // - Si user a 2+ blocs → ambigu, on NE LIE PAS (return { ambiguous: true }).
  //   La tache reste sans lien bloc → apparait dans 'A planifier' du user.
  async function planTaskInTranche(taskId, dateUnix, user, tranche) {
    ...
    if (profilBlocs.length >= 2) {
      return { ambiguous: true, count: profilBlocs.length };
    }
    const ordre = profilBlocs.length === 1 ? (profilBlocs[0].ordre || 1) : 1;
  ...
  window.WuboGrist = {
    ...
    formatBlocTime, TRANCHE_TO_ORDRE, ORDRE_TO_TRANCHE, TRANCHE_RANK
  };
})();
