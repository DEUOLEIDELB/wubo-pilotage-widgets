...
(function () {
  ...
  // Si le profil bloc cible est marque bloque=true, retourne { blocked: true } sans rien faire.
  async function planTaskInBloc(taskId, dateUnix, user, tranche, ordre = 1) {
    if (!taskId || !dateUnix || !user || !tranche) return null;
    // Verifie si le profil bloc est bloque
  ...
  // - Si 0 bloc disponible (vide ou tous bloques) : cree un bloc generique ordre=1
  //   (sauf si tous les profil blocs de la tranche sont bloques → ambiguous).
  ...
  async function planTaskInTranche(taskId, dateUnix, user, tranche) {
    ...
    const available = allInTranche.filter(p => !p.bloque);
    // Cas degenere : user a configure des blocs mais TOUS sont bloques.
