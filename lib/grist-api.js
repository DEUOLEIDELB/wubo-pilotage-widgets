...
(function () {
  ...
  // Auto-injection createur + modifie_par + date_modification.
  ...
  async function addRecord(tableId, fields) {
    ...
    // Pour Blocs_temps : auto-derive tranche_ordre depuis tranche si absent
    if (tableId === 'Blocs_temps' && enriched.tranche && !enriched.tranche_ordre) {
      const ordre = TRANCHE_TO_ORDRE[enriched.tranche];
      if (ordre) enriched = { ...enriched, tranche_ordre: ordre };
    }
    if (inGristIframe()) {
  ...
  // ============ Profils blocs : config tranches par user ============
  // Mapping tranche label -> ordre. Pont entre profils users avec labels libres.
  // 'Matin' = ordre 1, 'Aprem' = ordre 2, 'Soir' = ordre 3.
  ...
  // Renvoie les blocs config d'un user, triés par ordre
  async function getProfilBlocs(user) {
    const all = await fetchProfilsBlocs();
    return all.filter(p => p.user === user).sort((a, b) => (a.ordre || 0) - (b.ordre || 0));
  }

  // Pour un bloc Blocs_temps donné, calcule label/heures à afficher.
  // Priorité : override du bloc > profil du user > fallback hardcode.
  // Reçoit profilBlocs (tableau du user) en paramètre pour éviter le re-fetch.
  function getBlocDisplay(bloc, profilBlocs) {
    const ordre = bloc.tranche_ordre || TRANCHE_TO_ORDRE[bloc.tranche] || 0;
    const profilBloc = (profilBlocs || []).find(p => p.ordre === ordre);
    const label = profilBloc ? profilBloc.label : (bloc.tranche || `Bloc ${ordre}`);
    ...
    return { label, debut, fin, isOverride, ordre };
  }
  ...
  // Générique pour Projets / Objectifs / Sujets_a_pousser (et Taches via archiveTask plus bas avec date_fait).
  async function archiveRecord(tableId, id) {
    await updateRecord(tableId, id, { statut: 'Archive' });
  }

  // Trouve ou cree un bloc pour user a une date+tranche precise. Retourne { blocId, created }.
  // Match strict sur tranche : si on demande Soir, on prend Soir (ou cree un Soir).
  ...
  // Plan une tache existante dans un bloc (user, date, tranche). Cree le bloc si absent.
  // Utilise par la zone "A planifier" de Jour/Semaine pour lier une tache flottante en un clic.
  async function planTaskInTranche(taskId, dateUnix, user, tranche) {
    if (!taskId || !dateUnix || !user || !tranche) return null;
    const { blocId, created } = await ensureBlocForUserAtDate(user, dateUnix, tranche);
    // Ajoute la tache aux taches_liees du bloc (si pas deja)
  ...
  window.WuboGrist = {
    ...
    ensureBlocForUserAtDate, planTaskInTranche,
    fetchProfilsBlocs, clearProfilsBlocsCache, getProfilBlocs, getBlocDisplay,
    formatBlocTime, TRANCHE_TO_ORDRE, ORDRE_TO_TRANCHE
  };
})();
