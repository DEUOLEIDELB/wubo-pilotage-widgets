...
(function () {
  ...
  function openProfileMenu() {
    ...
    return showModal((content, close) => {
      ...
      // ============ GOOGLE CALENDAR : connexion par profil ============
      // Token OAuth stocke par profil (Taki/Numa/Lyes). Chacun connecte SON propre
      // compte Google. Pas de croisement possible. Sync silencieuse au render.
      if (window.WuboGCal) {
        content.appendChild($el('label', { class: 'profile-section-label' }, 'Google Calendar'));
        const gcalRow = $el('div', { style: 'display:flex;flex-direction:column;gap:6px' });
        function paintGCalRow() {
          $clear(gcalRow);
          const connected = window.WuboGCal.isConnected(current);
          if (connected) {
            const email = window.WuboGCal.getEmail(current);
            gcalRow.appendChild($el('div', {
              style: 'font-size:12px;color:var(--text-dim);display:flex;gap:6px;align-items:center'
            },
              $el('span', { style: 'color:var(--success);font-size:14px' }, '●'),
              $el('span', {}, 'Connecté' + (email ? ' · ' + email : ''))
            ));
            // Bouton diagnostic : sync now sur les 30 prochains jours, retour visible.
            const actionsRow = $el('div', { style: 'display:flex;gap:6px;flex-wrap:wrap' });
            actionsRow.appendChild($el('button', {
              class: 'primary small',
              onclick: async () => {
                const _now = new Date();
                const startISO = new Date(_now.getFullYear(), _now.getMonth(), _now.getDate(), 0, 0, 0).toISOString();
                const endISO = new Date(_now.getFullYear(), _now.getMonth(), _now.getDate() + 30, 0, 0, 0).toISOString();
                try {
                  const res = await window.WuboGCal.syncWindow(current, startISO, endISO);
                  console.log('[gCal sync manuelle]', current, startISO, '→', endISO, res);
                  if (res.tokenExpired) {
                    WuboGrist.toast('Token expiré, reconnecte-toi', 'error');
                  } else if (res.errors && res.errors.length > 0) {
                    WuboGrist.toast(`Erreurs : ${res.errors.length} (voir console)`, 'error');
                  } else {
                    const total = res.added + res.updated + res.archived;
                    if (total === 0) {
                      WuboGrist.toast('Aucun event gCal sur 30j (voir console)', 'info', 3000);
                    } else {
                      WuboGrist.toast(`Sync : +${res.added} ↻${res.updated} ✕${res.archived}`, 'success', 3000);
                    }
                  }
                } catch (e) {
                  console.error('[gCal sync manuelle exception]', e);
            ...
            actionsRow.appendChild($el('button', {
              ...
              onclick: async () => {
                const ok = await confirm({
                  ...
                  message: 'Les tâches déjà importées restent dans Pilotage. Plus de nouvelle synchro tant que tu ne reconnectes pas.',
                  yesLabel: 'Déconnecter',
                  danger: true
                });
                if (ok) {
                  await window.WuboGCal.disconnect(current);
                  paintGCalRow();
                  WuboGrist.toast('Google Calendar déconnecté', 'info');
                }
              }
            }, 'Déconnecter'));
            gcalRow.appendChild(actionsRow);
          } else {
            gcalRow.appendChild($el('div', { class: 'profile-hint' },
              'Connecte ton compte Google pour voir tes RDV et events dans Aujourd\'hui et Semaine.'
            ));
            gcalRow.appendChild($el('button', {
              class: 'primary small',
              style: 'align-self:flex-start',
              onclick: async () => {
                try {
                  await window.WuboGCal.connect(current);
                  paintGCalRow();
                  WuboGrist.toast('Google Calendar connecté', 'success');
                } catch (e) {
                  WuboGrist.toast('Connexion refusée : ' + e.message, 'error');
                }
              }
