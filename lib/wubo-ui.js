...
(function () {
  ...
  function openProfileMenu() {
    ...
    return showModal((content, close) => {
      ...
      content.appendChild($el('div', { class: 'profile-hint' },
        'Personnalise tes tranches : nombre, label, horaires. La position dans la liste sert de pont entre profils (1 = matin pour tout le monde, peu importe le label).'
      ));
      const blocsListEl = $el('div', { class: 'profil-blocs-list' });
      content.appendChild(blocsListEl);

      // State : array de { id?, ordre, label, debut_heure, fin_heure, actif, _new?, _deleted?, _dirty? }
      let blocsState = [];

      function paintBlocs() {
        while (blocsListEl.firstChild) blocsListEl.removeChild(blocsListEl.firstChild);
        const visible = blocsState.filter(b => !b._deleted);
        visible.forEach((b, idx) => {
          ...
          blocsListEl.appendChild(row);
        });
        const addBtn = $el('button', {
          class: 'ghost small',
          style: 'border-style:dashed;align-self:flex-start;margin-top:6px',
          onclick: () => {
            blocsState.push({
              user: current,
        ...
        blocsListEl.appendChild(addBtn);
      ...
      function moveBloc(idxInVisible, delta) {
        const visible = blocsState.filter(b => !b._deleted);
        const target = idxInVisible + delta;
        if (target < 0 || target >= visible.length) return;
        // Swap dans blocsState (en gardant les _deleted en place)
      ...
      async function saveBlocs() {
        const visible = blocsState.filter(b => !b._deleted);
        // Recompute ordre selon position visible
        visible.forEach((b, idx) => {
        ...
        try {
          ...
          for (const b of visible) {
            if (b._new) {
              const fields = {
                user: current, ordre: b.ordre, label: b.label,
          ...
          // Recharge l'état pour reset les flags _new/_dirty
