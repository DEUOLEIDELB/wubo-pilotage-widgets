      ...
      function paintBlocs() {
        ...
        TRANCHES_ORDRE.forEach(tranche => {
          ...
          visible.forEach((b, idx) => {
            const rowWrap = $el('div', { class: 'profil-bloc-row-wrap' + (b.bloque ? ' bloque' : '') });
            ...
            // Badge recurrence compact, visible des qu'une recurrence est definie.
            const recurSummary = (WuboGrist.summarizeRecurrence
              ...
              : '');
            if (recurSummary) {
              rowWrap.appendChild($el('div', { class: 'profil-bloc-recur-badge' },
            ...
            blocLabel.appendChild($el('span', {}, 'Bloqué (pas de tâches auto)'));
      ...
      async function saveBlocs() {
        ...
        try {
          ...
          for (const b of blocsState.filter(x => !x._deleted)) {
            // Format Grist : ChoiceList ['L', 'Lun', ...] ou null si vide. Dates : unix ou null.
