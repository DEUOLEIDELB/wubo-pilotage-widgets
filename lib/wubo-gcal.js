...
(function () {
  ...
  async function fetchEvents(user, timeMin, timeMax) {
    ...
    if (!cals) return null;
    // Filtre : on prend uniquement les calendriers que l'user a "selected" (coches)
    // dans gCal. Cela exclut "Numeros de semaine", calendriers d'autres users non-coches,
    // calendriers d'abonnement type "Phases de la Lune", etc. Le user controle via gCal
    // ce qui apparait dans Pilotage (cocher/decocher).
    const visibleCals = cals.filter(c =>
      !c.hidden && !c.deleted && c.selected !== false
    );
    const allEvents = [];
  ...
  function eventToTaskFields(event, user) {
    ...
    return {
      ...
      // Stocke le calendrier source (utile pour pouvoir supprimer/update via le bon
      // calendarId plus tard). _calendarId vient du tagging dans fetchEvents.
      gcal_calendar_id: event._calendarId || 'primary'
    };
  }
  ...
  async function syncWindow(user, startISO, endISO) {
    ...
    for (const [eid, ev] of byEventId.entries()) {
      ...
      if (existing) {
        ...
        // Backfill gcal_calendar_id si manquant (rows creees avant cette colonne)
        if (!existing.gcal_calendar_id && fields.gcal_calendar_id) {
          patch.gcal_calendar_id = fields.gcal_calendar_id;
        }
  ...
  async function pushTask(user, task, calendarId = 'primary') {
    ...
    // Si create, on stocke event_id ET calendar_id cote Grist
    if (!isUpdate && eventId && task.id) {
      try {
        await window.WuboGrist.updateRecord('Taches', task.id, {
