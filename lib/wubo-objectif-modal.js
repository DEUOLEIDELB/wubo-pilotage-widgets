    ...
    return new Promise(resolve => {
      ...
      if (mode === 'edit' && objectif && objectif.id) {
        ...
        stickySave.appendChild($el('button', {
          class: 'ghost small',
          onclick: async () => {
            // Archive directe + cooldown undo 8s. Snapshot du statut precedent
            // pour pouvoir restorer si l'user clique Annuler dans la barre.
            const prevStatut = objectif.statut || 'Actif';
            try {
              await WuboGrist.archiveRecord('Objectifs', objectif.id);
              const titre = objectif.titre || 'Objectif';
              WuboUI.cooldownNotice({
                message: `« ${titre} » archivé`,
                durationMs: 8000,
                onCancel: async () => {
                  try {
                    await WuboGrist.updateRecord('Objectifs', objectif.id, { statut: prevStatut });
